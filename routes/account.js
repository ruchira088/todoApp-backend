...
const {ROUTES: {RESOURCES}} = require("../constants")
                ...
                database.collection(USERS_COLLECTION).find({username}).limit(1).toArray()
                    ...
                    .then(() =>
                    {
                        response.status(200).json({
                            ...
                            token: userTokens.addUser(username)
                        })
                    })
                    .catch(error =>
                    {
        ...
        .patch(multer().single(IMAGE_FIELD_NAME),
            ...
            ({username, body, file: {originalname, mimetype, buffer}}, response) =>
            {
                ...
                database.collection(RESOURCE_COLLECTION).insertOne(resourceDocument)
                    .then(() =>
                    {
                        const document = Object.assign({}, body, {imageFile: `${RESOURCES}/${resourceId}/${originalname}`})
                        return userCollection.updateOne({username}, {$set: document})
                    })
                    .then(() => userCollection.find({username}).limit(1).toArray())
                    .then(([user]) => response.status(200).json(user))
                    .catch(() => response.status(500).json({error: "Unable to update account."}))
            }
        )
